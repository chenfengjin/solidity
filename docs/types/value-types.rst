...
.. note::
    If you store a variable of ``address`` type and plan to send Ether to it, then
    store it as ``address payable`` to make this requirement visible. Also try to pull this
    type as far out as possible or do the conversion as early as possible.
